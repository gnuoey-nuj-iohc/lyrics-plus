...
const LyricsCache = {
  ...
  DB_VERSION: 6,  // TMI 캐시 추가 (버전 업데이트)
...
const Utils = {
  ...
  async showUpdateNotificationIfAvailable() {
    try {
      ...
      const disableUpdateNotification = StorageManager.getItem("ivLyrics:visual:disable-update-notification");
      if (disableUpdateNotification === "true" || disableUpdateNotification === true) {
        console.log("[ivLyrics] Update notifications are disabled");
        return {
          hasUpdate: false,
          currentVersion: this.currentVersion,
          latestVersion: this.currentVersion,
