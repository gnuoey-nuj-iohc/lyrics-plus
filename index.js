...
const Prefetcher = {
  ...
  async _prefetchTranslations(trackInfo, lyrics) {
    ...
    // 발음만 활성화되어 있고 번역이 없으면 번역도 자동으로 요청 (사용자 편의)
    const shouldAutoEnableTranslation = needPhonetic && !needTranslation && 
      (displayMode1 === "gemini_romaji" || displayMode2 === "gemini_romaji");
    ...
    const prefetchPromise = (async () => {
      try {
        ...
        if (needPhonetic) {
          try {
            // Fallback: Perplexity 먼저 시도, 실패 시 Gemini로 전환
            let phoneticResponse;
            const perplexityKey = StorageManager.getItem("ivLyrics:visual:perplexity-api-key");
            if (perplexityKey && perplexityKey.trim() && window.Translator?.callPerplexity) {
              try {
                phoneticResponse = await window.Translator.callPerplexity({
                  trackId,
                ...
                console.warn(`[Prefetcher] Perplexity failed, falling back to Gemini:`, perplexityError.message);
        ...
        // 발음만 활성화되어 있으면 번역도 자동으로 요청
        if (needTranslation || shouldAutoEnableTranslation) {
          try {
            // Fallback: Perplexity 먼저 시도, 실패 시 Gemini로 전환
            ...
            if (perplexityKey && perplexityKey.trim() && window.Translator?.callPerplexity) {
              try {
                translationResponse = await window.Translator.callPerplexity({
                  trackId,
                  artist: trackInfo.artist,
                  title: trackInfo.title,
                  text,
...
class LyricsContainer extends react.Component {
      ...
      if (needPhonetic) {
        ...
        if (perplexityKey && perplexityKey.trim() && window.Translator?.callPerplexity) {
          try {
            ...
            console.warn(`[Lyrics] Perplexity failed, falling back to Gemini:`, perplexityError.message);
            phoneticResponse = await Translator.callGemini({
              trackId,
              artist: this.state.artist || lyricsState.artist,
              title: this.state.title || lyricsState.title,
  ...
  lyricsSource(lyricsState, mode) {
    ...
    // 발음과 번역이 모두 표시되도록 자동 활성화 (사용자 편의)
    const hasPhonetic = displayMode1 === "gemini_romaji" || displayMode2 === "gemini_romaji";
    const hasTranslation = displayMode1 === "gemini_ko" || displayMode2 === "gemini_ko";
    
    // 발음만 있으면 번역도 추가
    if (hasPhonetic && !hasTranslation) {
      if (!displayMode1 || displayMode1 === "none") {
        displayMode1 = "gemini_ko";
      } else if (!displayMode2 || displayMode2 === "none") {
  ...
  getGeminiTranslation(lyricsState, lyrics, mode) {
    return new Promise((resolve, reject) => {
      ...
      const inflightPromise = (async () => {
        if (perplexityKey && perplexityKey.trim() && window.Translator?.callPerplexity) {
          try {
            return await window.Translator.callPerplexity({
            ...
            console.warn(`[Lyrics] Perplexity failed, falling back to Gemini:`, perplexityError.message);
            return await Translator.callGemini({
              apiKey,
              artist: this.state.artist || lyricsState.artist,
              title: this.state.title || lyricsState.title,
              text,
