...
1. [GitHub Releases](https://github.com/gnuoey-nuj-iohc/lyrics-plus/releases)에서 최신 버전을 다운로드합니다.
...
3. 고급 탭에서 API 키를 입력합니다:
   * **Gemini API 키**: [Google AI Studio](https://aistudio.google.com/apikey?hl=ko)에서 무료로 발급받을 수 있습니다.
   * **Perplexity API 키** (선택사항): [Perplexity AI](https://www.perplexity.ai/settings/api)에서 발급받을 수 있습니다. Perplexity API 키가 있으면 먼저 사용하고, 실패 시 Gemini API로 자동 전환됩니다.
...
* **병합**: 원본 ivLyrics v3.0.1의 최신 변경사항 병합
  * 전체화면 개선 (대기열 패널 감지 범위 조정, 애니메이션 추가)
  * 전역 단축키 지원 (`GlobalShortcuts.js` 추가)
  * 대규모 리브랜딩 및 UI 개선
